...
class MainTestCase(unittest.TestCase):
  ...
  def _do_request_with_inputs_v1(self, action, parameters):
  ...
  def _do_request_with_inputs_v2(self, action, parameters):
    return self._do_request({
        'queryResult': {
            'parameters': parameters,
            'action': action
  ...
  # Performs a v1 request to get a pre-defined response with the given action
  # and parameters, and returns the response string, checking that the response
  # displayText and speech response are the same.
  def _get_response_text_v1(self, action, parameters):
    response = self._do_request_with_inputs_v1(action, parameters)
  ...
  # Performs a v2 request to get a pre-defined response with the given action
  # and parameters, and returns the response string.
  def _get_response_text_v2(self, action, parameters):
    response = self._do_request_with_inputs_v2(action, parameters)
    message = json.loads(response.data)
    response_text = message['fulfillmentText']
    return response_text

  # Performs a v1 request to get an event with the given action and parameters,
  # and returns the name and data for the event.
  def _get_response_event_v1(self, action, parameters):
    response = self._do_request_with_inputs_v1(action, parameters)
    message = json.loads(response.data)
  ...
  # Performs a v2 request to get an event with the given action and parameters,
  # and returns the name and data for the event.
  def _get_response_event_v2(self, action, parameters):
    response = self._do_request_with_inputs_v2(action, parameters)
    message = json.loads(response.data)
    name = message['followupEventInput']['name']
    data = message['followupEventInput']['parameters']
    return name, data
  ...
  def test_result_not_in_request(self):
    ...
    self._test_bad_request(response,
                           'Neither "result" nor "queryResult" was provided ' +
                           'in the request body.')
  ...
  def test_get_quote_response(self):
    response_text = self._get_response_text_v1('get_quote_response', {})
    self.assertIn('Here is a quote by', response_text)

  def test_get_quote_response_by_author(self):
    response_text = self._get_response_text_v1(
        'get_quote_response',
        {'author': 'Grace Hopper'})
  ...
  def test_get_quote_response_by_author_and_topic_v2(self):
    response_text = self._get_response_text_v2(
        'get_quote_response',
        {'author': 'Grace Hopper',
         'topic': 'mathematics'})
    self.assertIn('Here is a quote by Grace Hopper', response_text)
  ...
  def test_get_quote_response_unrecognized_parameter(self):
    response = self._do_request_with_inputs_v1('get_quote_response',
                                               {'diameter': 4})
    self._test_bad_request(response,
  ...
  def test_get_quote_event(self):
    name, data = self._get_response_event_v1('get_quote_event', {})
  ...
  def test_get_quote_event_by_author(self):
    name, data = self._get_response_event_v1(
        'get_quote_event',
        {'author': 'Grace Hopper'})
    self.assertEqual('respond_with_quote', name)
    self.assertTrue(data['quote'])
  ...
  def test_get_quote_event_by_author_and_topic_v2(self):
    name, data = self._get_response_event_v2(
        'get_quote_event',
        {'author': 'Grace Hopper',
         'topic': 'mathematics'})
    self.assertEqual('respond_with_quote', name)
  ...
  def test_get_bio_response_v2(self):
    response_text = self._get_response_text_v2(
        'get_bio_response',
        {'author': 'Grace Hopper'})
    self.assertIn('computer scientist', response_text.lower())
  ...
  def test_get_bio_response_unrecognized_parameter(self):
    response = self._do_request_with_inputs_v1('get_bio_response',
                                               {'author': 'Grace Hopper',
                                                'diameter': 4})
    self._test_bad_request(response,
                           'Unrecognized parameter in request: diameter')
  ...
  def test_get_bio_response_missing_author_parameter(self):
    response = self._do_request_with_inputs_v1('get_bio_response', {})
    self._test_bad_request(response,
                           'No author parameter provided in request for a ' +
  ...
  def test_get_bio_event_v2(self):
    name, data = self._get_response_event_v2(
        'get_bio_event',
        {'author': 'Grace Hopper'})
    self.assertEqual('respond_with_bio', name)
  ...
  def test_get_bio_event_no_match(self):
    ...
    self.assertEqual('bio_not_found', name)
    self.assertNotIn('bio', data)

  def test_get_bio_event_unrecognized_parameter(self):
    response = self._do_request_with_inputs_v1('get_bio_event',
                                               {'author': 'Grace Hopper',
                                                'diameter': 4})
    self._test_bad_request(response,
                           'Unrecognized parameter in request: diameter')
